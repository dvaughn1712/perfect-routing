...
public class SysProcess : Closeable {
	...
	public init(_ cmd: String, args: [String]?, env: [(String,String)]?) throws {
		...
		cArgs[0] = strdup(cmd)
		cArgs[cArgsCount + 1] = UnsafeMutablePointer<CChar>((nilLiteral: ()))
		var idx = 0
		...
		cEnv[cEnvCount] = UnsafeMutablePointer<CChar>((nilLiteral: ()))
		idx = 0
		while idx < cEnvCount {
		...
		var action = posix_spawn_file_actions_t.init(nilLiteral: ())
		...
		var procPid = pid_t()
		let spawnRes = posix_spawnp(&procPid, cmd, &action, UnsafeMutablePointer<posix_spawnattr_t>((nilLiteral: ())), cArgs, cEnv)
